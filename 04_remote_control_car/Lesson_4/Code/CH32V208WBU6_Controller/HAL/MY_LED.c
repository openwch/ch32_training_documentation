...
void HAL_MyLEDInit(void)
{
    ...
    //GPIO Init
    GPIO_InitStructure.GPIO_Pin = MY_LED_POWER_STATE_PIN|MY_LED_LINK_STATE_PIN; //POWER_STATE_PIN-PA0,LINK_STATE_PIN-PA1
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;                        //推挽输出
    GPIO_Init(GPIOC, &GPIO_InitStructure);
    GPIO_ResetBits(GPIOC, MY_LED_POWER_STATE_PIN);                          //默认电平为低电平，电源指示LED点亮
    GPIO_SetBits(GPIOC, MY_LED_LINK_STATE_PIN);                             //默认电平为高电平，连接指示LED熄灭
}
