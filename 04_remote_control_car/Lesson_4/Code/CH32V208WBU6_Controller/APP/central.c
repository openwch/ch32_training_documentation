...
enum
{
    ...
    BLE_DISC_STATE_CHAR1, // Characteristic1 discovery
    BLE_DISC_STATE_CHAR3, // Characteristic3 discovery
...
// Peer device address
static uint8_t PeerAddrDef[B_ADDR_LEN] = {0x2B, 0x3A, 0x88, 0x26, 0x3B, 0x38};
...
static uint16_t centralChar1Hdl = 0;
static uint16_t centralChar3Hdl = 0;
...
static void centralCommandFrameCreate(uint8_t cmd, uint8_t *pValue, uint8_t len);
static uint8_t centralChecksumCalculate(uint8_t *pValue, uint8_t len);
/*********************************************************************
 * PROFILE CALLBACKS
 */
...
uint16_t Central_ProcessEvent(uint8_t task_id, uint16_t events)
{
    ...
    if(events & START_WRITE_CHAR1_EVT)
    {
        if(centralProcedureInProgress == FALSE)
        {
            // Do a write
            ...
            PRINT("START_WRITE_CHAR1_EVT\n");
            req.cmd = FALSE;
            req.sig = FALSE;
            req.handle = centralChar1Hdl;
            req.len = CEN_COMMAND_SLEEP_FRAME_LEN;
            req.pValue = GATT_bm_alloc(centralConnHandle, ATT_WRITE_REQ, req.len, NULL, 0);
            if(req.pValue != NULL)
            {
                centralCommandFrameCreate(CEN_COMMAND_SLEEP_ENABLE, req.pValue, req.len);

                if(GATT_WriteCharValue(centralConnHandle, &req, centralTaskId) == SUCCESS)
                {
                    centralProcedureInProgress = TRUE;
                    if (keyChange.keys == HAL_KEY_SW_1)
                    {
                        tmos_stop_task(halTaskID, HAL_JOYSTICK_EVENT);              //停止采集摇杆AD事件
                        tmos_clear_event(halTaskID, HAL_JOYSTICK_EVENT);
                        tmos_stop_task(centralTaskId, START_WRITE_CHAR3_EVT);       //停止向小车通知摇杆数据事件
                        tmos_clear_event(centralTaskId, START_WRITE_CHAR3_EVT);
                    }
                    else if (keyChange.keys == HAL_KEY_SW_2)
                    {
                        tmos_set_event(halTaskID, HAL_JOYSTICK_EVENT);                                  //启动采集摇杆AD事件
                        tmos_start_task(centralTaskId, START_WRITE_CHAR3_EVT, MS1_TO_SYSTEM_TIME(100)); //启动向小车通知摇杆数据事件
                    }
                    keyChange.keys = 0;
                }
                else
                {
                    GATT_bm_free((gattMsg_t *)&req, ATT_WRITE_REQ);
                    tmos_start_task(centralTaskId, START_WRITE_CHAR1_EVT, MS1_TO_SYSTEM_TIME(50));
                }
            }
        }
        else
        ...
        return (events ^ START_WRITE_CHAR1_EVT);
    }

    if(events & START_WRITE_CHAR3_EVT)
    {
        if(centralProcedureInProgress == FALSE)
        {
            // Do a write
...
//            PRINT("START_WRITE_CHAR3_EVT\n");
            req.cmd = FALSE;
            req.sig = FALSE;
            req.handle = centralChar3Hdl;
            req.len = CEN_COMMAND_CONTROL_FRAME_LEN;
            req.pValue = GATT_bm_alloc(centralConnHandle, ATT_WRITE_CMD, req.len, NULL, 0);
            if(req.pValue != NULL)
            {
                centralCommandFrameCreate(CEN_COMMAND_DRIVE_CONTROL, req.pValue, req.len);
        ...
        tmos_start_task(centralTaskId, START_WRITE_CHAR3_EVT, MS1_TO_SYSTEM_TIME(50));
        return (events ^ START_WRITE_CHAR3_EVT);
    }
    ...
    switch(pEvent->gap.opcode)
    {
        ...
        case GAP_LINK_ESTABLISHED_EVENT:
        {
            ...
            if(pEvent->gap.hdr.status == SUCCESS)
            {
                ...
                HAL_MyLEDState(MY_LED_LINK_STATE_PIN, ENABLE);
                ...
                tmos_start_task(halTaskID, HAL_KEY_EVENT, MS1_TO_SYSTEM_TIME(500));                  //启动按键扫描事件
        ...
        case GAP_LINK_TERMINATED_EVENT:
        {
            ...
            tmos_stop_task(halTaskID, HAL_KEY_EVENT);                   //停止按键扫描事件
            ...
            HAL_MyLEDState(MY_LED_LINK_STATE_PIN, DISABLE);             //连接状态灯熄灭
...
static void centralStartDiscovery(void)
{
    ...
    centralSvcStartHdl = centralSvcEndHdl = centralChar1Hdl = centralChar3Hdl = 0;
...
static void centralGATTDiscoveryEvent(gattMsgEvent_t *pMsg)
{
    ...
    else if(centralDiscState == BLE_DISC_STATE_CHAR1)
    {
        ...
        {
            centralChar1Hdl = BUILD_UINT16(pMsg->msg.readByTypeRsp.pDataList[0],
...
//            tmos_start_task(centralTaskId, START_READ_OR_WRITE_EVT, DEFAULT_READ_OR_WRITE_DELAY);
        ...
        {
            // Discover characteristic
            centralDiscState = BLE_DISC_STATE_CHAR3;
            req.startHandle = centralSvcStartHdl;
            req.endHandle = centralSvcEndHdl;
            req.type.len = ATT_BT_UUID_SIZE;
    ...
    else if(centralDiscState == BLE_DISC_STATE_CHAR3)
    {
        // Characteristic found, store handle
        if(pMsg->method == ATT_READ_BY_TYPE_RSP &&
           pMsg->msg.readByTypeRsp.numPairs > 0)
        {
            centralChar3Hdl = BUILD_UINT16(pMsg->msg.readByTypeRsp.pDataList[0],
                                          pMsg->msg.readByTypeRsp.pDataList[1]);

            // Display Characteristic 1 handle
            PRINT("Found Characteristic 3 handle : %x \n", centralChar3Hdl);
        }
        if((pMsg->method == ATT_READ_BY_TYPE_RSP &&
            pMsg->hdr.status == bleProcedureComplete) ||
           (pMsg->method == ATT_ERROR_RSP))
...
/**************************************************************************************************
 * @fn      peripheralStateInfoReport
 *
 * @brief   Report peripheral state information
 *
 * @param   cmd - notify data type
 *          dev_num - number of the device that get the data
 *
 * @return  Checksum
 **************************************************************************************************/
static void centralCommandFrameCreate(uint8_t cmd, uint8_t *pValue, uint8_t len)
{
    switch(cmd)
    {
    case CEN_COMMAND_SLEEP_ENABLE:
        if(len == CEN_COMMAND_SLEEP_FRAME_LEN)
        {
            pValue[0] = CEN_COMMAND_FRAME_START;
            pValue[1] = (uint8_t)CEN_COMMAND_SLEEP_FRAME_LEN;
            pValue[2] = (uint8_t)(CEN_COMMAND_SLEEP_FRAME_LEN>>8);
            pValue[3] = CEN_COMMAND_SLEEP_ENABLE;
            pValue[4] = (uint8_t)(CEN_COMMAND_SLEEP_FRAME_LEN-CEN_COMMAND_BASIC_FRAME_LEN);
            pValue[5] = (uint8_t)((CEN_COMMAND_SLEEP_FRAME_LEN-CEN_COMMAND_BASIC_FRAME_LEN)>>8);
            if(keyChange.keys == HAL_KEY_SW_1)
            {
                pValue[6] = ENABLE;
            }
            ...
            pValue[7] = centralChecksumCalculate(&pValue[1], CEN_COMMAND_SLEEP_FRAME_LEN - 3);
            pValue[8] = CEN_COMMAND_FRAME_END;
        }
        break;

    case CEN_COMMAND_DRIVE_CONTROL:
        if(len == CEN_COMMAND_CONTROL_FRAME_LEN)
        {
            pValue[0] = CEN_COMMAND_FRAME_START;
            pValue[1] = (uint8_t)CEN_COMMAND_CONTROL_FRAME_LEN;
            pValue[2] = (uint8_t)(CEN_COMMAND_CONTROL_FRAME_LEN>>8);
            pValue[3] = CEN_COMMAND_DRIVE_CONTROL;
            pValue[4] = (uint8_t)(CEN_COMMAND_CONTROL_FRAME_LEN-CEN_COMMAND_BASIC_FRAME_LEN);
            pValue[5] = (uint8_t)((CEN_COMMAND_CONTROL_FRAME_LEN-CEN_COMMAND_BASIC_FRAME_LEN)>>8);
            pValue[6] = (JoystickState.LeftJoystickState.dir<<7) | JoystickState.LeftJoystickState.per;
            pValue[7] = (JoystickState.RightJoystickState.dir<<7) | JoystickState.RightJoystickState.per;
            pValue[8] = centralChecksumCalculate(&pValue[1], CEN_COMMAND_CONTROL_FRAME_LEN - 3);
            pValue[9] = CEN_COMMAND_FRAME_END;
        }
        break;
...
/**************************************************************************************************
 * @fn      centralChecksumCalculate
 *
 * @brief   Calculate buffer checksum
 *
 * @param   pValue - Pointer of data buffer
 *          len - Buffer length
 *
...
static uint8_t centralChecksumCalculate(uint8_t *pValue, uint8_t len)
{
    ...
    for (i = 0; i < len; ++i)
    {
        check_sum = (uint8_t)(check_sum + pValue[i]);
    }
